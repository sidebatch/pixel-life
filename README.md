...
- 오래된 숲 1-1에는 참나무·소나무·자작나무, 안쪽 1-2에는 나머지 5종을 밀도 있게 배치. 두 숲의 모든 나무는 기본 5회 타격, 해당 통나무 1~3개, 현실 시간 5분 후 재생
- 타격 후 나무 체력바의 남은 영역은 체력 비율에 따라 색이 바뀌고, 깎인 영역은 검정색으로 늘어난다. 마지막 타격 직후에는 전체가 검정색이다.
- 타격 중 체력 퍼센트 문구는 표시하지 않는다. 나무를 완전히 벴을 때만 벌목 XP(숲 1-1의 3종은 10, 숲 1-2의 5종은 25)를 받고 +XP 바·레벨업 알림이 나타난다. XP와 레벨은 저장된다. 수치는 임시 밸런스다.
...
- 벌목 도끼 등급·상점 업그레이드·목재 가격·전용 모션의 상세 데이터 확정 및 연결
