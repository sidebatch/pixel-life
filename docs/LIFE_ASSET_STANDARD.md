...
| 작물별 어린 잎 4종 | `assets/farming/young/*.png` | 96×96 | 성장 중간 단계 |
...
중간 성장 그림 4종은 게임의 완성 작물 그림을 스타일 참고로 새로 제작했다. 당근은 잔잎, 감자는 넓은 잎, 옥수수는 긴 잎, 딸기는 낮게 퍼진 잎만 보이며 수확물은 드러나지 않는다. 심은 직후의 덮인 흙과 첫 새싹은 공용 Canvas 픽셀 도형이라 별도 이미지 파일이 없다. 기존 완성 작물 그림을 축소해 성장 단계에 재사용하지 않는다.
