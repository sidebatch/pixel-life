...
- 낚싯대 정보 UI, 가방에서 직접 장착·저장과 입질 시간·희귀 이상 Weight·큰 개체 보정
- 월드 메뉴는 실제로 열리는 가방·낚싯대·도감만 표시하고, 가방은 주요 카드로 배치. 가방의 물고기·장비·재료는 모두 3열 이미지 카드로 표시하며 오른쪽 위 수량 배지와 커서·터치 이름 확인을 공통 사용 (물고기 개별 크기·판매가 기록은 저장 유지)
